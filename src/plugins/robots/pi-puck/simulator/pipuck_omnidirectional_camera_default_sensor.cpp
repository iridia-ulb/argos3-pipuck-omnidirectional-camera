#include "pipuck_omnidirectional_camera_default_sensor.h"
...
namespace argos {
   ...
   CPiPuckOmnidirectionalCameraDefaultSensor::CPiPuckOmnidirectionalCameraDefaultSensor() :
   ...
   void CPiPuckOmnidirectionalCameraDefaultSensor::SetRobot(CComposableEntity& c_entity) {
      try {
         m_pcControllableEntity = &(c_entity.GetComponent<CControllableEntity>("controller"));
         m_pcEmbodiedEntity = &(c_entity.GetComponent<CEmbodiedEntity>("body"));
      }
      catch(CARGoSException& ex) {
         THROW_ARGOSEXCEPTION_NESTED("Error setting robot for the Pi-Puck omnidirectional camera sensor", ex);
      }
   }
   ...
   void CPiPuckOmnidirectionalCameraDefaultSensor::Init(TConfigurationNode& t_tree) {
      try {
         /* initialize the parent class */
         CCI_PiPuckOmnidirectionalCameraSensor::Init(t_tree);
         /* show the frustum? */
         ...
         m_cProjectionMatrix(0,0) = CCI_PiPuckOmnidirectionalCameraSensor::m_cFocalLength.GetX();
         m_cProjectionMatrix(1,1) = CCI_PiPuckOmnidirectionalCameraSensor::m_cFocalLength.GetY();
         m_cProjectionMatrix(0,2) = CCI_PiPuckOmnidirectionalCameraSensor::m_cPrincipalPoint.GetX();
         m_cProjectionMatrix(1,2) = CCI_PiPuckOmnidirectionalCameraSensor::m_cPrincipalPoint.GetY();
         /* calculate fustrum constants */
         Real fWidthToDepthRatio = (0.5 * CCI_PiPuckOmnidirectionalCameraSensor::m_cResolution.GetX()) /
            CCI_PiPuckOmnidirectionalCameraSensor::m_cFocalLength.GetX();
         Real fHeightToDepthRatio = (0.5 * CCI_PiPuckOmnidirectionalCameraSensor::m_cResolution.GetY()) /
            CCI_PiPuckOmnidirectionalCameraSensor::m_cFocalLength.GetY();
         ...
         m_cOffset.SetFromComponents(
            CCI_PiPuckOmnidirectionalCameraSensor::m_cOrientationOffset,
            CCI_PiPuckOmnidirectionalCameraSensor::POSITION_OFFSET);
         /* get indices */
   ...
   void CPiPuckOmnidirectionalCameraDefaultSensor::Update() {
   ...
   void CPiPuckOmnidirectionalCameraDefaultSensor::Reset() {
      /* clear the LED cache */
      m_vecLedCache.clear();
      m_vecTags.clear();
      m_fTimestamp = 0.0;
      /* reset the base class */
      CCI_PiPuckOmnidirectionalCameraSensor::Reset();
   }

   /****************************************/
   /****************************************/

   bool CPiPuckOmnidirectionalCameraDefaultSensor::operator()(CTagEntity& c_tag) {
   ...
   bool CPiPuckOmnidirectionalCameraDefaultSensor::operator()(CDirectionalLEDEntity& c_led) {
   ...
   CPiPuckOmnidirectionalCameraDefaultSensor::ELedState
      CPiPuckOmnidirectionalCameraDefaultSensor::DetectLed(const CVector3& c_position) const {    
   ...
   CRadians CPiPuckOmnidirectionalCameraDefaultSensor::GetAngleWithCamera(const CPositionalEntity& c_entity) const {
   ...
   CVector2 CPiPuckOmnidirectionalCameraDefaultSensor::ProjectOntoSensor(const CVector3& c_point) const {
   ...
   bool CPiPuckOmnidirectionalCameraDefaultSensor::IsInsideFrustum(const CVector3& c_point) const {
   ...
   REGISTER_SENSOR(CPiPuckOmnidirectionalCameraDefaultSensor,
                   "pipuck_omnidirectional_camera", "default",
                   "Michael Allwright [allsey87@gmail.com]",
                   "1.0",
                   "The Pi-Puck omnidirectional camera sensor.",
                   "[TODO: Documentation]",
                   "Under development"
   );
}
